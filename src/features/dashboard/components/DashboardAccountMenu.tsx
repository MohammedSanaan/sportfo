  ...
  return (
    <div ref={containerRef} className="relative">
      <button
        ...
        aria-expanded={open}
        aria-haspopup="menu"
        className="flex items-center gap-2.5 rounded-full border border-border-default bg-surface-muted py-1 pr-3.5 pl-1 transition-colors hover:bg-brand-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500"
      >
        ...
        <span className="hidden text-left leading-tight sm:block">
          ...
          <span className="block text-[11px] tracking-[0.04em] text-ink-500">{roleLine}</span>
        </span>
      </button>
      ...
      {open && (
        <div
          role="menu"
          className="absolute right-0 z-30 mt-2 w-56 overflow-hidden rounded-xl border border-border-default bg-white py-1.5 shadow-xl"
        >
          <Link
            href="/athlete/profile"
            role="menuitem"
            onClick={() => setOpen(false)}
            className="block px-4 py-2.5 text-sm font-medium text-ink-700 transition-colors hover:bg-surface-muted"
          >
            {viewProfileLabel}
          </Link>
          {isAdmin && (
          ...
          <LogoutButton
            ...
            className="w-full rounded-none px-4 py-2.5 text-left text-sm font-medium text-red-600 transition-colors hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-50"
          />
        </div>
      )}
    </div>
